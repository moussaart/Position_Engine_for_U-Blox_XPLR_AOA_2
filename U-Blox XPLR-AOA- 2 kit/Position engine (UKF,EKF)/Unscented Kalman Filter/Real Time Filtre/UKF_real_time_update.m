...
function [SmoothStateOut,SmoothCovOut,Iout] = UKF_real_time_update(ukf,meas,F,SmoothState,SmoothCov,i)
% Prédiction de l'état et de la covariance avec le filtre UKF
[predictedState, predictedCov] = ukf.predict();

% Correction de l'état et de la covariance avec la mesure réelle actuelle
[filteredState, filteredCov] = correct(ukf, meas);  % X=[x, vx, y, vy], size(P) = (Size(X), Size(X))

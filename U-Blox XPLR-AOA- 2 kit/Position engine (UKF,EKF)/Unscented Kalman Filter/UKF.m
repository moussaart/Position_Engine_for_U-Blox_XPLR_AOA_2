...
function  [Xout,Yout ,mem_used]=UKF(X0,meassurment,ref_points,R,Q,dt,simulation,plot)
% Define the time interval between measurements
    ...
    %ekf=extendedKalmanFilter(@myStateTransitionFcn,@myMeasurementFcn);
    ...
    Yout = horzcat(Y(100:T),Ye(100:T), filteredStates(3,100:T)',filteredStates(3,100:T)');

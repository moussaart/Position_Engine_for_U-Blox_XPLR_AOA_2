...
% real_time_Com is a MATLAB function for real-time processing of sensor data
% from multiple serial ports and updating position estimates using Unscented
% Kalman Filtering (UKF) and Extended Kalman Filtering (EKF) techniques.

function real_time_Com(port, port1 ,port2 ,port3,x0,Q,X_anchor,Y_anchor,orientation,sigma_theta)
...
ekf = Inititaliztion_filter_EKF(x0,sigma_theta,Q,ref_points,dt);

% Initialize some variables (not clear from the provided code what they are used for)
SmoothState_ukf = zeros(4, 1);
SmoothCov_ukf = zeros(4);
smooth_test_ukf = 0;
SmoothState_ekf = zeros(4, 1);
SmoothCov_ekf = zeros(4);
smooth_test_ekf = 0;
...
[X_e, X_f_ukf, X_eWindow, X_f_ukf_Window] =Plot0(subplot(2, 2, 1),maxWindowSize, "X en m",'Estimated:UKF', x_lim);
hold on;
    X_f_ekf = plot(NaN, NaN, 'g--', 'LineWidth', 1,"DisplayName",'Estimated:EKF');
hold off;
X_f_ekf_Window = zeros(1, maxWindowSize);

% Call the custom function 'Intialization_plot' to create the second subplot (bottom-left)
[Y_e, Y_f_ukf, Y_eWindow, Y_f_ukf_Window] = Plot0(subplot(2, 2, 3), maxWindowSize, "Y en m",'Estimated:UKF', y_lim);

hold on;
    Y_f_ekf = plot(NaN, NaN, 'g--', 'LineWidth', 1,"DisplayName",'Estimated:EKF');
hold off;
Y_f_ekf_Window = zeros(1, maxWindowSize);
...
plot_XF_ukf = plot(0, 0, 'ro');%for UKF
plot_XF_ekf = plot(0, 0, 'go');%for EKF
      ...
      % Main program loop (starting from time t=2)
        ...
        for t=2:inf
            ...
            if all(TESTS)
                ...
                [SmoothState_ukf, SmoothCov_ukf, smooth_test_ukf] = UKF_real_time_update(ukf, meas, ...
                                                                             F, ...
                                                                             SmoothState_ukf, ...
                                                                             SmoothCov_ukf, ...
                                                                             smooth_test_ukf);
                % Real-time update of Extended Kalman Filter (EKF) states
                [SmoothState_ekf, SmoothCov_ekf, smooth_test_ekf] = EKF_real_time_update(ekf, meas, ...
                                                                             F, ...
                                                                             SmoothState_ekf, ...
                                                                             SmoothCov_ekf, ...
                                                                             smooth_test_ekf);
                ...
                x_f_ukf = SmoothState_ukf(1);
                y_f_ukf = SmoothState_ukf(3);
                x_f_ekf = SmoothState_ekf(1);
                y_f_ekf = SmoothState_ekf(3);
                % Update display windows for X and Y plots
                timeWindow = [timeWindow(2:end), t*dt];
                [X_eWindow, X_f_ukf_Window] = mise_a_jour_plot(X_eWindow, xe, ...
                                                             X_f_ukf_Window, x_f_ukf, ...
                                                             X_e, X_f_ukf, timeWindow);

                X_f_ekf_Window = [X_f_ekf_Window(2:end), x_f_ekf];
    
                % Update data in each subplot
                set(X_f_ekf, 'XData', timeWindow, 'YData', X_f_ekf_Window);
    
                [Y_eWindow, Y_f_ukf_Window] = mise_a_jour_plot(Y_eWindow, ye, ...
                                                             Y_f_ukf_Window, y_f_ukf, ...
                                                             Y_e, Y_f_ukf, timeWindow);
                Y_f_ekf_Window = [Y_f_ekf_Window(2:end), y_f_ekf];
    
                % Update data in each subplot
                set(Y_f_ekf, 'XData', timeWindow, 'YData', Y_f_ekf_Window);

                set(plot_XE, 'XData', xe, 'YData', ye);
                set(plot_XF_ukf, 'XData', x_f_ukf, 'YData', y_f_ukf);
                set(plot_XF_ekf, 'XData', x_f_ekf, 'YData', y_f_ekf);
